"""
temel_analiz.py — Temel analiz metrikleri ve finansal oranlar.
yFinance + borsapy entegrasyonu ile BIST ve yabancı hisseler için analiz.
...
_SEKTOR_LISTESI: Dict[str, Dict[str, str]] = {}
_SEKTOR_JSON = os.path.join(os.path.dirname(os.path.abspath(__file__)), "sektor_listesi.json")


def _sektor_listesi_yukle() -> None:
    """Sektör listesini JSON dosyasından yükle."""
    ...
    try:
        ...
        log.debug(f"Sektör listesi yüklendi: {len(_SEKTOR_LISTESI)} şirket")
    except FileNotFoundError:
        log.warning(f"Sektör listesi dosyası bulunamadı: {_SEKTOR_JSON}")
        _SEKTOR_LISTESI = {}
    except json.JSONDecodeError as e:
        log.error(f"Sektör listesi JSON parse hatası: {e}")
        _SEKTOR_LISTESI = {}
    except Exception as e:
        log.exception(f"_sektor_listesi_yukle genel hata: {e}")
        _SEKTOR_LISTESI = {}
...
def _sektor_bul(ticker_symbol: str) -> str:
    ...
    Args:
        ticker_symbol: Hisse sembolü (örn: "THYAO.IS")
    
    Returns:
        Sektör adı veya boş string
    """
    ...
    if sektor:
        log.debug(f"Sektör JSON'dan bulundu: {t} → {sektor}")
        return sektor
    ...
    if ticker_symbol.upper().endswith(".IS"):
        try:
            ...
            if sektor:
                ...
                log.debug(f"Sektör borsapy'den çekildi: {t} → {sektor}")
        except ImportError:
            log.debug("borsapy bulunamadı, sektör bilgisi atlandı")
        except Exception as e:
            log.debug(f"_sektor_bul hatası ({t}): {e}")
    
    return sektor


def _sektordeki_hisseler(sektor: str, hisse_kodu: str) -> List[str]:
    """
    Aynı sektördeki diğer hisseleri döner.
    
    Args:
        sektor: Sektör adı
        hisse_kodu: Ana hisse sembolü (listeden çıkarılacak)
    
    Returns:
        Aynı sektördeki diğer hisse kodları listesi
    """
    _sektor_listesi_yukle()
    ...
    if len(_SEKTOR_LISTESI) < 10:
        try:
            ...
            log.debug(f"Sektör listesi borsapy'den güncellendi: {len(_SEKTOR_LISTESI)} şirket")
        except ImportError:
            log.debug("borsapy bulunamadı, şirket listesi atlandı")
        except Exception as e:
            log.debug(f"_sektordeki_hisseler borsapy hatası: {e}")

    sonuc = [
        k for k, v in _SEKTOR_LISTESI.items()
        if v.get("sector", "") == sektor and k != t
    ]
    log.debug(f"Sektördeki hisse sayısı ({sektor}): {len(sonuc)}")
    return sonuc
...
def _sektörel_karsilastirma(hisse_kodu: str, sektor: str) -> Dict[str, Any]:
    """
    Aynı sektördeki hisselerin F/K, PD/DD, FD/FAVÖK ortalamalarını hesaplar.
    
    Args:
        hisse_kodu: Ana hisse sembolü
        sektor: Sektör adı
    
    Returns:
        Sektör ortalamaları dict
    """
    if not sektor:
        return {}
    ...
    try:
        ...
        log.debug("borsapy bulunamadı, sektörel karşılaştırma atlandı")
        return {}

    diger_hisseler = _sektordeki_hisseler(sektor, hisse_kodu)
    if not diger_hisseler:
        log.debug(f"Sektörde karşılaştırılabilir hisse bulunamadı: {sektor}")
        return {}

    fk_list: List[float] = []
    pddd_list: List[float] = []
    fdfavok_list: List[float] = []

    def _cek(ticker: str) -> Optional[Dict[str, Optional[float]]]:
        try:
            ...
            log.debug(f"Sektör veri çekme hatası ({ticker}): {e}")
            return None
    ...
    sonuc: Dict[str, Any] = {"_sektor_hisse_sayisi": len(diger_hisseler)}
    ...
    log.debug(f"Sektörel karşılaştırma tamamlandı: {sektor} ({len(fk_list)} F/K, {len(pddd_list)} PD/DD)")
    return sonuc
...
def _carpan_dogrula(yf_deger: float, bp_deger: float, alan_adi: str,
                    esik_pct: float = 10.0) -> Optional[Dict[str, Any]]:
    """
    yFinance ve borsapy değerini karşılaştırır.
    
    Args:
        yf_deger: yFinance değeri
        bp_deger: borsapy değeri
        alan_adi: Alan adı (F/K, PD/DD, vb.)
        esik_pct: Tolerans yüzdesi
    
    Returns:
        Fark > esik_pct% ise uyarı dict, aksi halde None
    """
    try:
        ...
        log.debug(f"_carpan_dogrula hatası ({alan_adi}): {e}")
    return None


def _borsapy_verileri(ticker_symbol: str, yf_info: Optional[Dict] = None) -> Dict[str, Any]:
    """
    borsapy'den: fiili dolaşım, yabancı oranı, analist hedefleri, ana ortaklar.
    
    Args:
        ticker_symbol: Hisse sembolü
        yf_info: yFinance info dict (opsiyonel, karşılaştırma için)
    
    Returns:
        borsapy'den çekilen ek veriler dict
    """
    ...
    try:
        ...
        if yf_info:
            ...
            kontroller = [
                (yf_fk, bp_fk, "F/K", 15.0),
                (yf_pddd, bp_pddd, "PD/DD", 15.0),
                (yf_mkcap, bp_mkcap, "Piyasa Değeri", 5.0),
        ...
        try:
            ...
            log.debug(f"Analist hedefleri hatası ({t}): {e}")
        ...
        try:
            ...
            log.debug(f"Ana ortaklar hatası ({t}): {e}")

        log.debug(f"borsapy verileri çekildi: {t} ({len(sonuc)} alan)")
        return sonuc

    except ImportError:
        log.debug("borsapy bulunamadı, ek veriler atlandı")
        return {}
    except Exception as e:
        log.exception(f"_borsapy_verileri genel hata ({ticker_symbol}): {e}")
        return {}
...
def get_val(df: pd.DataFrame, row_names: List[str], col_index: int = 0) -> float:
    """
    DataFrame'den güvenli değer okuma.
    
    Args:
        df: pandas DataFrame
        row_names: Aranacak satır isimleri (ilk bulunan kullanılır)
        col_index: Sütun indeksi
    
    Returns:
        Değer veya 0.0 (bulunamazsa)
    """
...
def safe_div(pay: float, payda: float, multiply: float = 1, fallback: float = 0.0) -> float:
    """
    Güvenli bölme işlemi (sıfıra bölme korumalı).
    
    Args:
        pay: Bölünen
        payda: Bölen
        multiply: Çarpım faktörü (örn: 100 için yüzde)
        fallback: Hata durumunda döndürülecek değer
    
    Returns:
        Bölüm sonucu veya fallback
    """
    try:
...
def pct_change(yeni: float, eski: float) -> float:
    """
...
def calc_beta(ticker_symbol: str, stock_returns: pd.Series, period: str = "1y") -> float:
    """
    Beta katsayısı hesaplama (piyasaya göre hassasiyet).
    
    Args:
        ticker_symbol: Hisse sembolü
        stock_returns: Hisse getiri serisi
        period: Dönem ("1y" veya "2y")
    
    Returns:
        Beta katsayısı veya 0.0
    """
    try:
        ...
        if len(m) < 30:
            log.debug(f"Benchmark veri yetersiz: {benchmark} ({len(m)} bar)")
            return 0.0
        
        df = pd.concat([stock_returns, m], axis=1, join="inner").dropna()
        if len(df) < 30:
            log.debug(f"Beta için yeterli ortak veri yok: {len(df)} bar")
            return 0.0
        ...
        beta = round(cov / var, 3) if var and var > 0 else 0.0
        log.debug(f"Beta ({period}) hesaplandı: {ticker_symbol} → {beta}")
        return beta
    
    except Exception as e:
        log.debug(f"calc_beta hatası ({ticker_symbol}, {period}): {e}")
        return 0.0
...
def temel_analiz_yap(ticker_symbol: str) -> Dict[str, Any]:
    """
    Temel analiz metriklerini hesapla ve döndür.
    
    Args:
        ticker_symbol: Hisse sembolü (örn: "THYAO.IS", "AAPL")
    
    Returns:
        Dict with all fundamental analysis metrics
    
    Example:
        >>> sonuc = temel_analiz_yap("THYAO.IS")
        >>> print(sonuc["F/K (Hesaplanan)"])
        12.45
    """
    try:
        log.debug(f"Temel analiz başlatılıyor: {ticker_symbol}")
        hisse = taze_ticker(ticker_symbol)

        def _fetch(attr: str) -> Tuple[str, Any]:
        ...
        if bs is None or bs.empty or inc is None or inc.empty:
            log.warning(f"Finansal veri bulunamadı: {ticker_symbol}")
        ...
        # ── Çeyreklik Veriler ────────────────────────────────────────────────
        def _q_ilk_dolu(df_q: pd.DataFrame, row_names: List[str]) -> Tuple[float, int]:
            for i in range(4):
        ...
        try:
            ...
            log.debug(f"Price history hatası ({ticker_symbol}): {e}")
        ...
        # N. Ham Referans Değerler (iç kullanım)
        ...
        log.debug(f"Temel analiz tamamlandı: {ticker_symbol} ({len(s)} metrik)")
        return s

    except Exception as e:
        log.exception(f"temel_analiz_yap genel hata ({ticker_symbol}): {e}")
        return {"Hata": f"Temel analiz yapılamadı: {str(e)}"}
