...
async def komut_uyari_sil(message: Message):
    ...
    try:
        ...
        await uyari_sil(uyari_id, user_id=message.from_user.id)
...
async def callback_temel(callback: CallbackQuery):
    ...
    # ✅ GÜVENLİK: Callback data'dan gelen sembolü doğrula
    valid, sembol, _ = validate_symbol(sembol)
    if not valid:
        await callback.answer("❌ Geçersiz sembol.", show_alert=True)
        return
