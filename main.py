...
def blok(baslik: str, emoji: str, satirlar: list) -> str:
    """
    Temiz HTML bölüm bloğu.
    satirlar: [(anahtar, deger), ...] ya da ["satır1", "satır2", ...]
    """
    if not satirlar:
        return ""
    icerik_lst = []
...
def ma_blok(teknik: dict) -> str:
    """Hareketli ortalamalar için özel formatlı blok."""
    satirlar = []
    for tip in ("SMA (Basit)", "EMA (Üstel)", "WMA (Ağırlıklı)"):
        if tip in teknik:
            kisalt = tip.split()[0]
            deger = teknik[tip]
            # Her periyodu alt satıra böl (5g:xxx | 20g:xxx → 5g:xxx  20g:xxx)
...
def komut_haber(message):
    ...
    _son_istek[user_id] = datetime.now()
    bekle_msg = bot.reply_to(message,
        f"⏳ 📰 {bold(girdi)} haberleri çekiliyor...", parse_mode="HTML")
    ...
    try:
        ...
        if komut in ("analiz","ai"):
            with ThreadPoolExecutor(max_workers=2) as ex:
                ...
                temel_v = ft.result(); teknik_v = fk.result()
        elif komut == "temel":
        ...
        if temel_v and "Hata" in temel_v:
            _gonder(chat_id, mesaj_id, f"❌ {h(temel_v['Hata'])}"); return
        if teknik_v and "Hata" in teknik_v:
            _gonder(chat_id, mesaj_id, f"❌ {h(teknik_v['Hata'])}"); return
        ...
        if teknik_v:
            ...
            for k, v in teknik_v.items():
                if k.startswith("_") or k in MA_KEYS: continue
                v_str = _fmt(v)
        ...
        if komut == "ai" and temel_v and teknik_v:
            bot.send_message(chat_id,
                ...
                parse_mode="HTML")
            # Finnhub haberleri varsa AI'a ekle
        ...
        try: _gonder(chat_id, mesaj_id, hata, duzenle=True)
        except: bot.send_message(chat_id, hata, parse_mode="HTML")
...
def _piyasa_isle(chat_id, mesaj_id, girdi, tip):
    try:
        if tip == "kripto":   piyasa, teknik = kripto_analiz(girdi)
        elif tip == "doviz":  piyasa, teknik = doviz_analiz(girdi)
        else:                 piyasa, teknik = emtia_analiz(girdi)
        ...
        for i, msg in enumerate(mesajlar):
            if i == 0:
                try: bot.edit_message_text(msg, chat_id=chat_id,
                    message_id=mesaj_id, parse_mode="HTML")
        ...
        except: bot.send_message(chat_id, f"❌ Hata: {h(str(e))}", parse_mode="HTML")
